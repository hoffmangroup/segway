...
*--minibatch-fraction=FRAC* option, which will cause Segway to use a random 
fraction FRAC of genomic positions at each training iteration.  This will 
allow training to have access to the whole genome for training while 
maintaining fast iterations.  Note that using this option will select on the 
basis of windows, so it is best to combine --minibatch-fraction with 
--split-sequences.  The likelihood-based training stopping criterion 
is no longer valid with minibatch training, so training will always run to 
--max-train-rounds (100, by default) if --minibatch-fraction is set.
